...
export async function fetchCurrentPeriod(): Promise<CurrentPeriodResponse & { isMock?: boolean }> {
  try {
...
export async function fetchArchive(page = 1, perPage = 20): Promise<ArchiveListResponse> {
  return apiFetch<ArchiveListResponse>(`/api/v1/periods/archive?page=${page}&per_page=${perPage}`)
}
